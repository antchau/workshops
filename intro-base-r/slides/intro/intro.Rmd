...
- R code will be surrounded by a light yellow box
...
- R syntax is straightforward

- Represent text by surrounding in a pair of double quotes or single quotes. Choose one and be consistent

.code-bg[
```{r text,collapse=TRUE}
"this is text"
'this is text too'
```
]

- To distinguish from actual text, keep objects, functions, argument names (input names) unquoted

.code-bg[
```{r non-text,collapse=TRUE,eval=FALSE}
# mean is a function
# c is a function
# x is an argument name
mean(x = c(1,2,3))
```
]
