...
- When subsetting a data frame with a single index `df[]` is similar to list subsetting
...
# Subset a data frame - single index
...
mascots <- data.frame(name = c("Peter Anteater", "Josephine Bruin", 
                      ...
                      age = c(56, 101, 60, 140), gpa = c(4, 3.9, 3.87, 3.7),
...
- All methods of vector subsetting are valid for data frames
...
# Subset a data frame - double index
...
# Create a new column - simple case
...
- Often, we would like to see a subset of our data given some condition
- The general syntax to filter using subsetting operator:
...
# Select and filter data with `subset()`
