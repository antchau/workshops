...
```{r knit-note1, include=FALSE}
# use slides/tables/data-types.txt when knit to html

# use tables/data-types.txt when using pagedown::chromeprint(rmarkdown::render("path_to_rmd'))
```

<br>  

.center[
`r knitr::asis_output(paste(readLines('tables/data-types.txt'), collapse = '\n'))`
]
...
.center[
**Vector Coercion rule:** character → double → integer → logical
]
...
```{r knit-note2, include=FALSE}
# use slides/tables/matrix-functions.txt when knit to html

# use tables/matrix-functions.txt when using pagedown::chromeprint(rmarkdown::render("path_to_rmd'))
```
...
.center[
`r knitr::asis_output(paste(readLines('tables/matrix-functions.txt'), collapse = '\n'))`
]
