...
- R can infer the argument name based on the order you provide the arguments
- There is a tradeoff between being concise and explicit
- Let's take a look at the built-in correlation function
...
# note that I don't need to specify x and y explicitly
# the first argument is x, the second argument is y
cor(c(1,2,3), c(-1, 3, 4))
```
]
...
- This is useful for functions which intuitvely work with unlimited arguments

.code-bg[
```{r unlimited-argument,collapse=TRUE,eval=FALSE}
# look at function definition
# sum(..., na.rm = FALSE)
